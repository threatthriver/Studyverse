...
export default function PomodoroTimer() {
  ...
  const handleTimerEnd = useCallback(() => {
    ...
    if (mounted && Notification.permission === "granted") {
      try {
        ...
        // Fallback for environments where Notification API might be restricted (e.g., iframes, specific browser settings)
        toast({
          title: notificationTitle,
          description: notificationBody,
        });
      }
    } else if (mounted) { // If permission not granted or denied, use toast as fallback
        toast({
          title: notificationTitle,
          description: notificationBody,
        });
  ...
  const resetTimer = () => {
    ...
    setMode("work");
    setTimeLeft(WORK_DURATION);
    console.log("Timer reset. Settings could be opened here.");
  };
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <CardTitle 
                ...
                onClick={() => console.log("Group timer options clicked")}
              >
                  Group timer <ChevronDown className="ml-1 h-4 w-4 opacity-70" />
              </CardTitle>
            </TooltipTrigger>
            <TooltipContent side="top"><p>Timer Options</p></TooltipContent>
          </Tooltip>
        </CardHeader>
        <CardContent className="flex flex-col items-center space-y-2 p-3">
          <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-1">
                  <Tooltip>
                    ...
                    <TooltipContent side="right"><p>{isActive ? "Pause" : "Start"}</p></TooltipContent>
                  </Tooltip>
                  <Tooltip>
                    <TooltipTrigger asChild>
                    ...
                    <TooltipContent side="right"><p>Reset/Settings</p></TooltipContent>
                  </Tooltip>
              </div>
          </div>
